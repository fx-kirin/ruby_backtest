...
class BacktestOrderList
  ...
  def initialize
    ...
    rows = @db.execute <<-SQL
      CREATE TABLE IF NOT EXISTS backtest_current_order_list (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        symbol text,
        order_type int,
        lots int,
  ...
  def insert_order(order)
    ...
    result = @db.execute("INSERT INTO backtest_current_order_list(#{param}) VALUES(#{ques})", arg);
    order
  end
  ...
  def save(order)
    ...
    if order.status == 2
      @db.execute("DELETE FROM backtest_current_order_list WHERE order_number=?", order.order_number);
    else
    result = @db.execute("UPDATE backtest_current_order_list SET #{param} WHERE order_number=?", arg);
    end
  ...
  def get_open_positions
    positions = []
    param = set_select_param
    result = @db.execute("SELECT #{param} FROM backtest_current_order_list ");
    result.each{|data|
      positions << set_order_from_data(data)
    }
    positions
  ...
  def get_close_positions
    get_all_positions("WHERE status = 2")
  end
  
  def get_positions_by_magic_number(magic_number)
    get_all_positions("WHERE magic_number = "+magic_number.to_s)
  end
  
  def get_open_positions_by_magic_number(magic_number)
    positions = []
    param = set_select_param
    result = @db.execute("SELECT #{param} FROM backtest_current_order_list WHERE status = 1 AND magic_number = "+magic_number.to_s);
    result.each{|data|
      positions << set_order_from_data(data)
    }
    positions
  ...
  def get_close_positions_by_magic_number(magic_number)
    get_all_positions("WHERE status = 2 AND magic_number = "+magic_number.to_s)
  end
  
  def delete_all_positions
    @db.execute("DELETE FROM backtest_order_list");
    @db.execute("DELETE FROM backtest_current_order_list");
  end
