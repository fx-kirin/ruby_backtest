...
class BacktestOrderManager
  ...
  def close_order(order_number, close_price, close_time)
    order = get_open_position_from_order_number(order_number)
  ...
  def get_positions_by_magic_number(magic_number)
    @open_orders.get_positions_by_magic_number(magic_number)
  end
  
  def get_positions_by_order_number(order_number)
    @open_orders.get_positions_by_order_number(order_number)
  end
end